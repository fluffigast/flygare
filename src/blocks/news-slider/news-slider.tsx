        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-1.5">
            <button
              ...
              disabled={page === 0}
              className="w-9 h-9 grid place-items-center rounded-full transition-colors hover:bg-paper disabled:opacity-35 disabled:cursor-not-allowed"
          ...
          {Array.from({ length: pages }).map((_, i) => (
            <button
              ...
              className={`w-1.5 h-1.5 rounded-full transition-all ${
                i === page ? "bg-ink scale-130" : "bg-slate-2/40 hover:bg-slate-2"
              }`}
              aria-label={`Sida ${i + 1}`}
            />
          ))}
